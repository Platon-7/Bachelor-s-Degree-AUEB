...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        weight_tensor = torch.tensor(self.class_weights, device=device, dtype=torch.float32)
...
def main():
    ...
    if training_args.do_train:
        ...
        train_dataset = load_dataset('csv', data_files=dataset_path)
    ...
    # Convert train_dataset to a Pandas DataFrame
    train_dataframe = train_dataset.to_pandas()
    # Labels
    ...
    if config.model_type == 'deberta' and model_args.hierarchical:
        ...
        if model_args.hierarchical:
            ...
            # Freeze all layers
            # EDIT------------------------------------------------------------------------
            for param in segment_encoder.parameters():
                param.requires_grad = False
            # Unfreeze the last layer
            for param in segment_encoder.encoder.layer[-1].parameters():
    ...
    if training_args.do_train:
        ...
        # Step 1: Set your batch size and initialize the ImbalancedDatasetSampler
        #batch_size = training_args.per_device_train_batch_size
        #sampler = ImbalancedDatasetSampler(train_dataset)

        # Step 2: Initialize a DataLoader using the ImbalancedDatasetSampler
    ...
    gradient_clipping = "auto"  # or set it to a specific value like 1.0

    # Set the value for args.max_grad_norm
    training_args.max_grad_norm = 1.0  # adjust the value as needed
    ...
    if training_args.do_eval:
        ...
        print("metrics['eval_loss']:", metrics['eval_loss'])
